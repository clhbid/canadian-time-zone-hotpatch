...
describe("createTimeZoneHotpatch", () => {
  ...
  it("resolves with the same results as the package root", () => {
    ...
    expect(instance.resolveTimeZone(instant)).toEqual(resolveTimeZone(instant));
  ...
  it("labels results with supplemented and overridden translations", () => {
    ...
    expect(instance.resolveTimeZone({ ...input, locale: "fr-CA" }).label).toBe(
      "Heure de l'Alberta"
    );
    expect(instance.resolveTimeZone(input).label).toBe("Alberta Time (ABT)");
    expect(
    ...
    expect(resolveTimeZone({ ...input, locale: "fr-CA" }).label).toBe(
      "Alberta Time (ABT)"
    );
  });
