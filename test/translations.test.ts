...
describe("translations", () => {
  it("returns the base dictionary unchanged when there are no overrides", () => {
    expect(mergeTranslations(defaultTranslations, undefined)).toBe(defaultTranslations);
  });
  ...
  it("merges overrides without mutating the base dictionary", () => {
    ...
    expect(merged["fr-CA"]?.["ab-permanent-time-2026"]).toBe("Heure de l'Alberta");
  ...
  it("lets an override replace an existing label for the same locale and rule", () => {
    ...
    expect(merged["en-CA"]?.["bc-permanent-time-2026"]).toBe("Pacific Time (PCT)");
  });
